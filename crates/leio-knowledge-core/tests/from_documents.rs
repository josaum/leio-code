...
//!
//! The whole point of the constructor is that no repository is required, so
//! these tests carry their own bytes.

use leio_knowledge_core::{FormalStore, RdfFormat, sparql_with_store};

const TINY_GYM: &[u8] = include_bytes!("fixtures/tiny_gym.ttl");

fn grounded(envelope: &leio_knowledge_core::QueryEnvelope) -> Option<bool> {
...
fn builds_a_store_from_bytes_and_answers_grounded() {
    ...
    assert_eq!(grounded(&envelope), Some(true));
}
...
fn a_malformed_document_does_not_blind_the_graph() {
    ...
    // The skip is visible on every answer, not buried in stats.
    let envelope = sparql_with_store(&store, "SELECT ?s WHERE { ?s ?p ?o }", 1);
    assert!(
        envelope
            .warnings
            .iter()
            .any(|w| w.starts_with("rdf skip broken.ttl"))
    );
}

#[test]
fn unbound_question_is_a_refusal_not_an_empty_success() {
    let store = FormalStore::from_documents([("tiny_gym.ttl", RdfFormat::Turtle, TINY_GYM)])
        .expect("store from bytes");
    let envelope = sparql_with_store(
        &store,
        "PREFIX ex: <http://example.org/gym#> SELECT ?u WHERE { ?u ex:priceMonthly ?p }",
        10,
    );
    assert_eq!(grounded(&envelope), Some(false));
    assert_eq!(envelope.confidence, 0.0);
    assert!(envelope.warnings.iter().any(|w| w == "ungrounded"));
    let reasoning = envelope
        .meta
        .as_ref()
        .and_then(|m| m.get("reasoning"))
        .and_then(|r| r.as_array())
        .expect("refusal carries a reasoning trail");
    assert_eq!(reasoning[0]["kind"], "refuse");
}
