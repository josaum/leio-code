...
fn check_cli_surface(root: &Path, warnings: &mut Vec<String>, evidence: &mut Vec<EvidenceItem>) {
    ...
    for (needle, detail) in [
        ...
        (
            "SelfContract,",
            "CLI DoctorKind exposes the self-contract doctor",
        ),
        (
            "DoctorKind::SelfContract => \"self-contract\"",
            "CLI maps the self-contract doctor kind to the registered doctor name",
        ),
    ] {
        if let Some(line) = find_line(&src, needle) {
            evidence.push(EvidenceItem {
...
fn check_binary_freshness(warnings: &mut Vec<String>, evidence: &mut Vec<EvidenceItem>) {
    let baked = env!("LEIO_BUILD_COMMIT");
    let exe = std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("leio-code"));
